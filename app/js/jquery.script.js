...
jQuery(document).ready(function() { 
    ...
    (function ($) {
		...
		$(window).bind('resize', function () { 
			if ($(this).width() > 1024) {
				$('.cmsms_resp a.social_toggle').removeClass('current');
				
				$('.cmsms_resp .header_html').removeAttr('style');
				$('.cmsms_resp .wrap_social_icons ul').removeAttr('style');
			}
		} );
    } )(jQuery);
	...
	(function ($) { 
		if ($(window).width() <= 1024) {
			$('.no_responsive .search_but').bind('click', function () { 
				if ($(this).parents('.no_responsive .wrap_header_search').hasClass('open')) {
					$(this).parents('.no_responsive .wrap_header_search').removeClass('open');
				} else {
					$(this).parents('.no_responsive .wrap_header_search').addClass('open')
				}
	...
	(function ($) { 
		$(window).resize(function () { 
			if ($(window).width() <= 1024) {
				$('.cmsms_resp .wrap_header_search').addClass('open');
			} else {
				$('.cmsms_resp .wrap_header_search').removeClass('open');
			}
		} );
		if ($(window).width() <= 1024) { 
			$('.cmsms_resp .wrap_header_search').addClass('open');
	...
	(function ($) { 
		$('a.responsive_nav').bind('click', function () { 
			...
			setTimeout(function () { 
				if ($(window).height() < ($('.cmsms_resp #navigation').height() + 47)) {
					$('.cmsms_resp #header').css( { 
						position : 'relative', 
						top : 0 
					} );
					
					$('.cmsms_resp .header_inner').css( { 
						marginTop : 0 
					} );
				} else if ( 
					$(window).height() > ($('.cmsms_resp #navigation').height() + 47) && 
					$('.cmsms_resp #header').css('position') !== 'fixed' 
				) {
					$('.cmsms_resp #header').removeAttr('style');
					
					$('.cmsms_resp .header_inner').removeAttr('style');
				}
			}, 500);
		...
		$(window).bind('resize', function () { 
			if ($(this).width() > 1024) {
				$('.cmsms_resp a.responsive_nav').removeClass('active');
				
				$('.cmsms_resp #navigation').removeAttr('style');
				$('.cmsms_resp #navigation ul').removeAttr('style');
			} else {
				setTimeout(function () { 
					if ($(window).height() < ($('.cmsms_resp #navigation').height() + 47)) {
						$('.cmsms_resp #header').css( { 
	...
	(function ($) { 
		if ($(window).width() <= 768) {
			if ($('.cmsms_resp .cmsms_cc div.one_fourth:odd').next().is('.cl_js')) {
				$('.cl_js + .cl_js').remove();
			} else {
				$('.cmsms_resp .cmsms_cc div.one_fourth:odd').after('<div class="cl_js" />');
			}
		} else {
			$('.cl_js').remove();
		}
